...
export function renderToHTML(doc: DocumentNode, customStyles = ''): string {
...
function renderFrontmatter(meta: Frontmatter): string {
  if (!meta.title && !meta.author && !meta.level) return '';
  ...
  while (i < nodes.length) {
    ...
    if (node.type === 'list-item') {
      ...
      while (i < nodes.length && nodes[i].type === 'list-item') {
        listItems.push(nodes[i] as ListItemNode);
        i++;
      }
...
function renderDirective(node: DirectiveBlockNode): string {
...
function renderQuestion(node: QuestionNode): string {
  let optionsHTML = '';
